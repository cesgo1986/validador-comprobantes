...
// FIX: "fecha_banner" agregado al type
...
// FIX: interface completa con todos los campos del backend
interface Resultado {
  riesgo: RiskLevel;
  score: number;
        ...
        <div style={{ padding: "12px 24px", borderBottom: "1px solid #F0F4F8", display: "flex", gap: 10, flexWrap: "wrap" }}>
          ...
          {(result.score_ocr !== undefined || result.score_visual !== undefined) && (
            <div style={{ flex: 2, minWidth: 160, background: "#F8FAFC", borderRadius: 10, padding: "10px 12px", border: "1px solid #E2E8F0" }}>
              ...
              {([["OCR", result.score_ocr], ["Visual", result.score_visual], ["Contextual", result.score_contextual], ["Bancario", result.score_bancario]] as [string, number | undefined][])
...
export default function Home() {
  ...
  // FIX: Stage usa el type correcto, sin showFechaBanner
  ...
  // FIX: analyze recibe fechaConfirmada como parámetro directo (evita closure stale de useState)
  const analyze = async (fechaConfirmada = false) => {
    ...
    // FIX: enviar fecha_pasada_confirmada al backend usando el parámetro, no el estado
    ...
    try {
      ...
      // FIX: usar requiere_confirmacion_fecha del backend como fuente de verdad
      // Cubre todos los casos: ayer, semana pasada, meses atrás
  ...
  // FIX: reset sin showFechaBanner, resetea fechaPasadaConfirmada
  const reset = () => {
  ...
  // FIX: pasa true directamente a analyze, sin depender de setState asíncrono
