...
**Backend (código listo, 2026-07, pendiente de aplicar y desplegar):** `services/aggregation_service.py` — 3 agregaciones nuevas (`calcular_banco_mayor_incidencia`, `calcular_comparacion_volumen`, `calcular_comparacion_alertas`), ninguna requiere dato nuevo, todas cruzan relaciones/columnas que ya existen. `services/dashboard_service.py` — `obtener_centro_operativo()`, bundle completo calcado del wireframe de `DESIGN_SYSTEM.md` sección 10. `main.py` — endpoint nuevo `GET /api/v1/dashboard/centro-operativo`. **Pendiente: la pantalla de escritorio que consuma este endpoint** (probablemente dentro de `/perfil`, expandiendo el Executive Summary de 4.2).
