...
**Frontend (código listo, 2026-07, pendiente de aplicar y desplegar):** `app/components/perfil/CentroOperativo.tsx` (nuevo, puramente presentacional — recibe `datos` por prop, no hace fetch propio) — estructura calcada del wireframe de `DESIGN_SYSTEM.md` sección 10 (estado 🟢/🟠/🔴, hero stat, "qué requiere atención", "tendencias"; ningún bloque aparece si no hay nada real que decir). `app/perfil/page.tsx` hace **una sola llamada** a `/centro-operativo` y reparte el mismo objeto a ambas presentaciones vía `.vp-mobile-only`/`.vp-desktop-only` — ver `DECISION_LOG.md`, ADR "una sola llamada al backend para Mobile y Desktop" (corrección de arquitectura hecha antes de desplegar: el diseño original hacía 2 llamadas a 2 endpoints distintos, duplicando tráfico y creando 2 fuentes de verdad). Botones de acción navegan a `/alertas` o `/historial` (sin filtro preaplicado todavía — mejora futura, no bloqueante).

**Con esto, el frontend de 5.5 queda listo — pendiente de aplicar, desplegar y verificar antes de poder marcar 5.5 y Etapa 5 completas.**
