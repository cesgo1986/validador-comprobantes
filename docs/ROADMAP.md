...
| 6.2.6 ✅ | **Invitaciones reales — completo y verificado de punta a punta (2026-08).** No se usó el "Invite user" nativo del panel de Supabase (esa opción es para dar de alta manual desde el panel, no un flujo integrado a la app) — se construyó un flujo propio: `POST /usuarios/invitar` (backend, permiso `USERS`) + API administrativa de Supabase + `/usuarios` y `/aceptar-invitacion` (frontend). 4 bugs reales encontrados y corregidos durante la primera prueba real, ver `DECISION_LOG.md`: llave incorrecta (`sb_secret_` no sirve para este endpoint, se necesita `service_role` legacy), fila huérfana en `usuarios` cuando el correo fallaba, "Site URL" de Supabase sin actualizar, Redirect URL de la invitación faltante |
...
| 6.3.5 ✅ | Migrar endpoints reales a `require_permission(...)` + registrar actividad: `/analizar` (`OPERATE`), `/analisis/exportar` (`EXPORT`), `/alertas/{id}/estado` (`OPERATE`) — desplegado y verificado en producción, confirmadas filas reales en `activity_logs` con `metadata_json` correcto |

**Con esto, Etapa 6.3 (Access Control Layer) queda completa.**
